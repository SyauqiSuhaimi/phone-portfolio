...
const CodeBlock = ({ code, language, caption }: { code: string, language: string, caption?: string }) => (
...
const DemoBlock = ({ componentName, caption, height = "300px" }: { componentName: string, caption?: string, height?: string }) => {
    ...
    return (
        <div className="my-10">
            <div className={`w-full rounded-2xl overflow-hidden border border-gray-200 dark:border-white/10 shadow-inner bg-gray-100 dark:bg-[#0A0A0A] relative group isolate`} style={{ height }}>
...
const RenderBlock = ({ block }: { block: CaseStudyBlock }) => {
    switch (block.type) {
        case 'text':
            return <p className="text-lg leading-8 text-gray-700 dark:text-gray-300 font-serif mb-6">{block.content}</p>;
        case 'heading':
            return <h2 className="text-2xl font-bold mt-12 mb-6 font-sans tracking-tight">{block.content}</h2>;
        case 'image':
            return (
                <figure className="my-8">
                    <img src={block.src} alt={block.caption || ''} className="w-full rounded-xl shadow-lg" />
        ...
        case 'demo':
            return <DemoBlock componentName={block.componentName} caption={block.caption} height={block.height} />;
        default:
            return null;
    }
};
                ...
                <div className="relative h-[60vh] w-full overflow-hidden">
                    ...
                    <div className="absolute bottom-0 left-0 w-full p-6 pb-12 bg-gradient-to-t from-black via-black/50 to-transparent pt-32">
                        ...
                        >
                            ...
                            <p className="text-lg text-white/80 font-serif leading-relaxed line-clamp-2 max-w-2xl">
                                {study.subtitle}
                            </p>
                        </motion.div>
                    </div>
                    ...
                    <div className="max-w-2xl mx-auto">
                        ...
                        <div className="flex items-center gap-4 mb-10 pb-8 border-b border-gray-100 dark:border-white/10 text-sm text-gray-500">
                             <div className="flex items-center gap-2">
                                <div className="w-8 h-8 rounded-full bg-gray-200"></div> {/* Author Avatar Placeholder */}
                                <span className="font-semibold text-black dark:text-white">Syauqi Suhaimi</span>
                             </div>
                             <span>•</span>
